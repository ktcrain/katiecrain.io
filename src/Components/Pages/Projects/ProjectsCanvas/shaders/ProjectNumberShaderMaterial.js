...
const ProjectNumberShaderMaterial = {
  uniforms: uniforms,
  vertexShader: [
    ...
    // "vDisplace = snoise(vUv * noiseFreq + t) ;",
    // "p.z = position.z + normal.z * vDisplace * noiseAmp;",

    "float f = 0.35;",
    "float a = 0.2;",

    // Oscillate vertices up/down
    "p.x += (sin(p.x * f + t) * 0.5 + 0.5) * a;",

    // Oscillate vertices inside/outside
    "p.y += (sin(p.x * f + t) * 0.5 + 0.5) * a;",

    // Oscillate vertices inside/outside
    "p.z += (sin(p.x * f + t) * 0.5 + 0.5) * a;",
  ...
  fragmentShader: [
    ...
    // "vec3 color = mix(red, white, vDisplace);",
...
export default ProjectNumberShaderMaterial;
