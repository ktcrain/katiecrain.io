...
const DarkWaterShaderMaterial = {
  ...
  fragmentShader: [
    ...
    "f = 1.0 - clamp(sin(f * PI * 2.) * dot(vUv, vUv) * uRes.y / 30. + .2, 1., 0.5);",

    // add the darkness to the end of the tunnel
    "f *= sin(length(vUv) - .1);",

    "vec3 c = hsv;",
    "vec2 stripPos = vec2( 0.0, vNoiseDisp );",
    // "vec2 stripPos = vec2( 0.0, vUv.x/vUv.y );",
    "vec4 stripColor = texture2D( pallete, stripPos );",
    "stripColor *= pow(1.0-vNoiseDisp, 1.0);",
    // "c = mix(c, stripColor.rgb, 1.0);",
    "c = mix(c, vec3(f), 0.8);",

    // "vec3 darken = mix(c,black,0.5);",
    // "c = mix(c, darken, 0.3 * vNoiseDisp);",
    // "c = mix(c, darken, 0.5 * vPointRippleDisp);",
...
export default DarkWaterShaderMaterial;
